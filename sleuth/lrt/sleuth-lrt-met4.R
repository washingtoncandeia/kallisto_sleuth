...
s2c <- read.table(file.path(base_dir, "metadata", "metadata_4.txt"), header = TRUE, stringsAsFactors = FALSE)
head(s2c)
...
write.table(as.data.frame(head(sleuth_significant, 400)), sep = "\t", "zika_significant_4.txt", row.names = F)
