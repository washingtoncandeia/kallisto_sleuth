...
## 2-Criando tabela
# metadata_4.txt (GBS-rec x control).
...
so <- sleuth_fit(so)
so <- sleuth_wt(so, which_beta = 'conditioncontrol')
...
sleuth_table <- sleuth_results(so, 'conditioncontrol', test_type = 'wt', show_all = TRUE)
...
write.table(as.data.frame(head(sleuth_significant, 400)), sep = "\t", "zika_significant_wt_4.txt", row.names = FALSE)
